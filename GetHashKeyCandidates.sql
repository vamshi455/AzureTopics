...
BEGIN
	...
	SET @SQL = 'INSERT INTO #TMPGROUPDATA SELECT ''' + @SCHEMANAME + ''',''' + @TABLENAME  +  ''', ''' + @COLUMNNAME + ''' , COUNT(1) AS COUNTOFGROUP FROM (SELECT ' + @COLUMNNAME  + ', COUNT(1) AS COUNTOFGROUP FROM '+ @SCHEMANAME + '.' + @TABLENAME + ' GROUP BY ' + @COLUMNNAME + ') A  HAVING COUNT(1) BETWEEN 60 AND 200'
	PRINT(@SQL)
	EXEC(@SQL)
